...
namespace Filmoteka
{
    public partial class Form_CompanysDlg : Form
    {
        ...
        public void LoadDataInFiledsByRecordId(int rec_id)
        {
            if (ConnectToSQLiteDB() == 1)
            {
                ...
                using (SQLiteCommand fmd = con.CreateCommand())
                {
                    try
                    {
                        fmd.CommandText = @"SELECT name FROM company WHERE id=@l";
            ...
            if (ConnectToSQLiteDB() == 1)
            {
                ...
                {// новий запис
                    ...
                    using (SQLiteCommand fmd = con.CreateCommand())
                    {
                        try
                        {
                            ...
                            fmd.CommandText = @"INSERT INTO company (name) VALUES (@n)";
                ...
                {// редактування
                    using (SQLiteCommand fmd = con.CreateCommand())
                    {
                        try
                        {
                            fmd.CommandText = @"UPDATE company SET name=@n WHERE id=@a";
                            fmd.Parameters.Add("@n", System.Data.DbType.String, -1);
                            fmd.Parameters["@n"].Value = name;
