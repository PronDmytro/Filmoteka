...
namespace Filmoteka
{
    public partial class Form_Directors : Form
    {
        ...
        private void button_refresh_Click(object sender, EventArgs e)
        {
            if (ConnectToSQLiteDB() == 1)
            {
                ...
                try
                {
                    string sql = @"SELECT id as '#',name as 'ПІБ Режисера' FROM filmDirector"; // можливо треба поставити LIMIT ?
                    SQLiteDataAdapter adapter = new SQLiteDataAdapter(sql, con);
                    DataSet ds = new System.Data.DataSet();
                    adapter.Fill(ds, "filmDirector");
                ...
                if (ConnectToSQLiteDB() == 1)
                {
                    using (SQLiteCommand fmd = con.CreateCommand())
                    {
                        try
                        {
                            ...
                            fmd.CommandText = @"DELETE FROM filmDirector WHERE id = @l";
        ...
        private void button_edit_Click_1(object sender, EventArgs e)
        {
            ...
            Form_DirectorsDlg frm = new Form_DirectorsDlg();
