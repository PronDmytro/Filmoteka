...
namespace Filmoteka
{
    public partial class Form_Companys : Form
    {
        ...
        {
            Form_CompanysDlg frm = new Form_CompanysDlg();
                ...
                if (ConnectToSQLiteDB() == 1)
                {
                    using (SQLiteCommand fmd = con.CreateCommand())
                    {
                        try
                        {
                            ...
                            fmd.CommandText = @"DELETE FROM company WHERE id = @l";
        ...
        private void button_refresh_Click(object sender, EventArgs e)
        {
            if (ConnectToSQLiteDB() == 1)
            {
                ...
                try
                {
                    string sql = @"SELECT id as '#',name as 'Найменування компанії' FROM orders"; // можливо треба поставити LIMIT ?
